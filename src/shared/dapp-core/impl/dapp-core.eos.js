...
const fetchBalance = ({chain, account}) => async ({symbol, addresses}) => {
    ...
    // return await rpc.get_currency_balance(addresses.EOS, account.address, symbol)
    const balances = await rpc.get_currency_balance(addresses.EOS, account.address, symbol)

    return balances[0].split(' ')[0].replace('.', '')

    return _.size(balances) === 1 ? parseFloat(balances[0]) : 0
}
