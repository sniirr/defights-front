...
export const Bridge = ({supportedChains, supportedTokens}) => {
    ...
    const {balance, balanceStr} = useSelector(balanceSelector(fromChainKey, token.symbol))
    ...
    const amount = BigNumber.from(amountStr || '0')

    let currentTxFee

    const renderFeeText = () => {
        if (_.isNil(amount) || _.isEmpty(txFee)) return null
        ...
        currentTxFee = amount.mul(Math.round(feePct * 10000)).div(1000000)
                    ...
                    <div className="row center-aligned-row">
                        ...
                        <div className="slider">
                            <Slider disabled={disabled} marks={{0: '0%', 25: '25%', 50: '50%', 75: '75%', 100: '100%'}}
                                    value={balance.gt(0) ? amount.div(balance).mul(100) : 0}
